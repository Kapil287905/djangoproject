...
def placeorder(req):
    if req.method=="POST":
        ...
        total=0
        if product_id:
            cart_items = get_object_or_404(Carts, userid=user, productid__productid=product_id)
            print(cart_items)           
            subtotal = cart_items.qty * cart_items.productid.price
            total += subtotal
            cartdata.append({
                
                "productname": cart_items.productid.productname,
                "qty": cart_items.qty,
                "price": cart_items.productid.price,
                "subtotal": subtotal,
            })
        else:
